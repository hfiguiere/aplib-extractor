...
fn main() {
    ...
    match args.command {
        ...
        Command::Tree(args) => process_tree(&args),
    };
}
...
fn print_children_for(uuid: &str, tree: &Tree, library: &Library, indent: u32) {
    ...
    if let Some(children) = tree.get(uuid) {
        for child in children {
            ...
            .unwrap_or_default();
            println!("[{typ}] {name}");
            print_children_for(child, tree, library, indent + 2);
        }
    }
}
...
fn process_tree(args: &TreeArgs) {
    ...
    albums
        ...
        .for_each(|uuid| add_object(uuid, &mut tree, &library));
    println!("TOP LEVEL");
    print_children_for(TOP_LEVEL, &tree, &library, 2);
}
